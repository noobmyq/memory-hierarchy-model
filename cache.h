...
public:
    SetAssociativeCache(const std::string& cacheName, size_t num_sets, size_t ways)
        : name(cacheName), numSets(num_sets), numWays(ways), 
          accesses(0), hits(0), globalLruCounter(0) {
    ...
    double getHitRate() const { return accesses > 0 ? (double)hits / accesses : 0.0; }
