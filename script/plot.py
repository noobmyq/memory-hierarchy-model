#!/usr/bin/env python3
"""
Timestamp-Based Memory Simulator Results Visualizer

This script creates visualizations from timestamp-specific CSV files generated by the 
memory simulator results gatherer. It processes each timestamp experiment separately.
"""
...
def interactive_mode(timestamp_files, experiment_names):
    ...
    metric_columns = [col for col in df.columns if any(x in col for x in 
                                                   ['hit', 'miss', 'efficiency', 'accesses', 'cost'])]
    ...
    while True:
        try:
            metric_choice = int(input(f"Enter your choice (1-{len(metric_columns)}): "))
            if 1 <= metric_choice <= len(metric_columns):
                y_column = metric_columns[metric_choice - 1]
                break
            print(f"Invalid choice. Please enter a number between 1 and {len(metric_columns)}.")
        except ValueError:
            print("Invalid input. Please enter a number.")
...
def parse_args():
    ...
    parser.add_argument('-y', '--y_column', type=str, default='l1_tlb_hit_percentage',
                        help='Column to use for y-axis (the statistic to visualize)')
...
def main():
    ...
    if args.interactive:
        ...
        config_columns = args.config_columns
        filters = {}
        sort_bars = args.sort
    ...
    for timestamp in selected_timestamps:
        ...
        # Create title with timestamp and experiment name
