...
SimConfig parse_arguments(int argc, char* argv[]) {
    ...
    if (argc < 2 || (argc == 2 && std::string(argv[1]) == "--help")) {
        std::cout
            << "Usage: " << argv[0] << " <trace_file> [options]\n"
            << "Options:\n"
            << "  --phys_mem_gb <GB>         Physical memory size (default: "
               "1)\n"
            << "  --l1_tlb_size <entries>    L1 TLB size (default: 64)\n"
            << "  --l1_tlb_ways <ways>       L1 TLB associativity (default: "
               "4)\n"
            << "  --l2_tlb_size <entries>    L2 TLB size (default: 1024)\n"
            << "  --l2_tlb_ways <ways>       L2 TLB associativity (default: "
               "8)\n"
            ...
            << "  --l1_ways <ways>           L1 Cache associativity (default: "
               "8)\n"
            << "  --l1_line <bytes>          L1 Cache line size (default: 64)\n"
            << "  --l2_cache_size <bytes>    L2 Cache size (default: 262144)\n"
            << "  --l2_ways <ways>           L2 Cache associativity (default: "
               "16)\n"
            << "  --l2_line <bytes>          L2 Cache line size (default: 64)\n"
            << "  --l3_cache_size <bytes>    L3 Cache size (default: 8388608)\n"
            << "  --l3_ways <ways>           L3 Cache associativity (default: "
               "16)\n"
            << "  --l3_line <bytes>          L3 Cache line size (default: "
               "64)\n";
        throw std::invalid_argument(argc < 2 ? "Missing trace file"
                                             : "Help requested");
    }
    ...
    for (int i = 2; i < argc; i += 2) {
        ...
        if (flag == "--phys_mem_gb")
            config.phys_mem_gb = value;
        else if (flag == "--l1_tlb_size")
            config.tlb.l1_size = value;
        else if (flag == "--l1_tlb_ways")
            config.tlb.l1_ways = value;
        else if (flag == "--l2_tlb_size")
            config.tlb.l2_size = value;
        else if (flag == "--l2_tlb_ways")
            config.tlb.l2_ways = value;
        else if (flag == "--pwc_size")
            config.pwc.size = value;
        else if (flag == "--pwc_ways")
            config.pwc.ways = value;
        else if (flag == "--l1_cache_size")
            config.cache.l1_size = value;
        else if (flag == "--l1_ways")
            config.cache.l1_ways = value;
        else if (flag == "--l1_line")
            config.cache.l1_line = value;
        else if (flag == "--l2_cache_size")
            config.cache.l2_size = value;
        else if (flag == "--l2_ways")
            config.cache.l2_ways = value;
        else if (flag == "--l2_line")
            config.cache.l2_line = value;
        else if (flag == "--l3_cache_size")
            config.cache.l3_size = value;
        else if (flag == "--l3_ways")
            config.cache.l3_ways = value;
        else if (flag == "--l3_line")
            config.cache.l3_line = value;
        else {
...
int main(int argc, char* argv[]) try {
    ...
    PageTable page_table(physical_memory, cache_hierarchy, config.tlb.l1_size,
                         config.tlb.l1_ways, config.tlb.l2_size,
                         config.tlb.l2_ways, config.pwc.size, config.pwc.ways);
